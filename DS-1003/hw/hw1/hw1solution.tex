...
$Z \sim \cn (\alpha_i \mu_i + \alpha_j \mu_j,\ \alpha_i^2\Sigma_{i,i} + \alpha_j^2\Sigma_{j,j} + 2\alpha_i\alpha_j\Sigma_{i,j})$
\\

\item (Optional) Assume $W$ and $R$ are two Gaussian distributed random variables. Is $W+R$ still Gaussian? \\

\textit{(Answer)} No\\\

Proof:

Let $W \sim \cn (\mu, \Sigma)$ Now let $R = -W$. Then it follows that $R \sim \cn (\mu, \Sigma)$. However,
\begin{align*}
W+R &= W-W,\\
&= 0.
\end{align*}
...
\textit{(Proof that M exists)}\\

Let $V=[v_1, v_2, ..., v_k]$ Since $x^* \in S_v$, there exists some $\hat{w} \in S_v$ such that
\begin{align*}
x^* &= Mw = V\hat{w}.
\end{align*}

Since $\hat{x}$ is orthogonal to $S_v$,
\begin{align*}
V^T(w-x^*)=0.
\end{align*}
